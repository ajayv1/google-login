...
passport.use(new GoogleStrategy(
  ...
  callbackURL: 'https://honorable-wakeful-glade.glitch.me/auth/accepted',  
...
function gotProfile(accessToken, refreshToken, profile, done) {
    ...
    let dbRowID;  // temporary! Should be the real unique
    // key for db Row for this user in DB table.
    // Note: cannot be zero, has to be something that evaluates to
    // True.  
    
    if (emailId.endsWith('ucdavis.edu')) {
      dbRowID = 1;
      done(null, dbRowID); 
    } else { // invalid user
      dbRowID = 10;
...
passport.deserializeUser((dbRowID, done) => {
    ...
    let userData = {userData: "maybe data from db row goes here", isUserInvalid: false};
    
    if (dbRowID == 10) { // invalid user
      userData.isUserInvalid = true;
    }
